...
class FoursquareViewController: SFViewController<FoursquareNode>, CLLocationManagerDelegate, ASTableDataSource, ASTableDelegate, SFSearchBarDelegate, FoursquareDataManagerDelegate {
    ...
    func tableNode(_ tableNode: ASTableNode, willDisplayRowWith node: ASCellNode) {
        guard let node = node as? SFAnimatable else { return }
        let fadeAnimation = SFFadeAnimation(type: .inside)
        let zoomAnimation = SFZoomAnimation(type: .inside)
        node.animator.animations = [fadeAnimation, zoomAnimation]
        node.animator.start()
    }
